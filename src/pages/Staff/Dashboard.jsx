          ...
          <div className="mb-8">
            <nav className="flex space-x-8">
              <button
                ...
                className={`py-2 px-1 border-b-2 font-medium text-sm transition-colors duration-200 ${activeTab === 'orders' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-500 hover:text-gray-300 hover:border-gray-400'}`}
              >
                Orders Management
              </button>
              <button
                onClick={() => setActiveTab('menu')}
                className={`py-2 px-1 border-b-2 font-medium text-sm transition-colors duration-200 ${activeTab === 'menu' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-gray-500 hover:text-gray-300 hover:border-gray-400'}`}
              >
                Menu Management
              </button>
            </nav>
            ...
            <div className="space-y-8">
              ...
              {orders.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400">Orders for {user?.full_name} will appear here.</p>
                </div>
              ) : (
                  ...
                  {orders.map((order) => (
                    <div key={order.id} className="glass-morphism-strong rounded-xl p-6">
                      <div className="flex flex-col lg:flex-row justify-between items-start mb-4 gap-4">
                        <div>
                          <h4 className="text-lg font-semibold text-white">Order #{order.id.slice(0, 8)}</h4>
                          <p className="text-sm text-gray-400 font-medium">{formatStudentInfo(order.user)}</p>
                        ...
                        <div className="space-y-2">
                          {order.order_items.map((item) => (
                            <div key={item.id} className="flex justify-between items-center glass-morphism p-2 rounded border border-white/10">
                              <span className="text-gray-300">{item.menu_item.name} x {item.quantity}</span>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button onClick={() => updateOrderStatus(order.id, 'processing')} disabled={order.status !== 'pending' || isOrderUpdating(order.id, 'processing')} className="px-4 py-2 modern-button text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 text-sm">
                          {isOrderUpdating(order.id, 'processing') ? 'Updating...' : 'Start Processing'}
                        </button>
                        <button onClick={() => updateOrderStatus(order.id, 'ready')} disabled={order.status !== 'processing' || isOrderUpdating(order.id, 'ready')} className="px-4 py-2 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 text-sm" style={{ background: 'linear-gradient(135deg, rgba(34, 197, 94, 0.9) 0%, rgba(16, 185, 129, 0.9) 100%)' }}>
                          {isOrderUpdating(order.id, 'ready') ? 'Updating...' : 'Mark Ready'}
                        </button>
                        <button onClick={() => updateOrderStatus(order.id, 'completed')} disabled={order.status !== 'ready' || isOrderUpdating(order.id, 'completed')} className="px-4 py-2 glass-morphism hover:bg-white/10 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 text-sm border border-white/20">
            ...
            <div className="space-y-6">
              <div className="flex justify-end mb-6">
                <button
                  ...
                  style={{ background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.9) 0%, rgba(5, 150, 105, 0.9) 100%)', boxShadow: '0 0 20px rgba(16, 185, 129, 0.4)' }}
                >
                    ...
                    <div key={item.id} className="glass-card rounded-xl overflow-hidden">
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex items-center justify-between text-sm text-gray-500 mb-2">
                          <span className={`font-medium ${item.quantity_available <= 0 ? 'text-red-400' : item.quantity_available <= 5 ? 'text-yellow-400' : 'text-green-400'}`}>
                            Available: {item.quantity_available}
                          </span>
                          <span>Serves: {item.serves}</span>
                        </div>
                        <div className="flex space-x-2 mt-4">
                          <button onClick={() => { setEditingItem(item); setIsEditModalOpen(true); }} className="flex-1 flex items-center justify-center space-x-2 modern-button text-white px-4 py-2 rounded-lg transition-all duration-200">
                            <Edit className="w-4 h-4" />
                            <span>Edit</span>
                          </button>
                          <button onClick={() => handleDeleteMenuItem(item.id)} disabled={deletingMenuItem === item.id} className="flex-1 flex items-center justify-center space-x-2 text-white px-4 py-2 rounded-lg transition-all duration-200 disabled:opacity-50" style={{ background: 'linear-gradient(135deg, rgba(239, 68, 68, 0.9) 0%, rgba(220, 38, 38, 0.9) 100%)' }}>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-morphism-strong rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between p-6 border-b border-white/20">
              <h3 className="text-xl font-semibold text-white">{editingItem ? 'Edit Menu Item' : 'Add New Menu Item'}</h3>
              <button onClick={() => setIsEditModalOpen(false)} disabled={savingMenuItem} className="p-2 hover:bg-white/10 rounded-lg transition-colors disabled:opacity-50">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2"><DollarSign className="w-4 h-4 inline mr-1" />Price (₹) *</label>
                  <input
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2"><Users className="w-4 h-4 inline mr-1" />Serves *</label>
                  <input
                    type="number" min="1"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2"><Package2 className="w-4 h-4 inline mr-1" />Available Qty *</label>
                  <input
                    type="number" min="0"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Canteen Name</label>
                  <input
                    type="text"
                    value={user?.full_name || ''}
                    className="w-full px-3 py-2 modern-input rounded-lg bg-white/5 text-gray-400"
                    disabled readOnly
                  />
                </div>
              </div>
            ...
            <div className="flex space-x-4 p-6 border-t border-white/20">
              <button onClick={() => setIsEditModalOpen(false)} disabled={savingMenuItem} className="flex-1 px-4 py-2 glass-morphism border border-white/20 rounded-lg hover:bg-white/10 transition-colors disabled:opacity-50 text-white">
                Cancel
              </button>
              <button
                onClick={handleSaveMenuItem}
                disabled={savingMenuItem}
                className="flex-1 px-4 py-2 text-white rounded-lg transition-all disabled:opacity-50 flex items-center justify-center space-x-2"
                style={{ background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.9) 0%, rgba(5, 150, 105, 0.9) 100%)' }}
              >
